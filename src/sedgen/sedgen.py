...
"""To Do:
    ...
    simulation and the later filling of the interfaces and crystal size
    arrays are not too big. Could these differences be avoided to start
    with and thus work with only one representation?
"""
...
class SedGen():
    ...
    def fill_main_cystal_size_array(self, crystal_size_random):
        ...
        for i in range(len(self.minerals)):
            crystal_size_array[np.where(self.interface_array == i)[0]] = crystal_size_random[i]
